...
pub fn check_collide(
    ...
    let (collide_bounds, new_pos) = check_collide_bounds(
        &new_pos,
        collide_span,
        direction,
        world_info.world_width(),
    ...
    let (collide_obj, new_pos) = check_collide_objs(
        entity,
        old_pos,
        &new_pos,
        collide_span,
...
fn check_collide_objs(
    ...
    let func = |e: &Entity| -> bool {
        ...
        let (collide_obj, corrected_pos) = check_collide_obj(
            &pos,
            collide_span,
            direction,
            &obj2.pos,
    ...
    if collide {
        info!("check_collide_objs old_pos={} new_pos={}", old_pos, pos);
    }
...
fn check_collide_obj(
    ...
    if cx * direction.y.abs() < cy * direction.x.abs() {
        ...
        info!("1 pos1={pos1} pos2={pos2} dx={dx} dy={dy} total_span={total_span} direction={direction} cx={cx} cy={cy} corrected_pos={corrected_pos}");
    } else {
        ...
        info!("2 pos1={pos1} pos2={pos2} dx={dx} dy={dy} total_span={total_span} direction={direction} cx={cx} cy={cy} corrected_pos={corrected_pos}");
    }
